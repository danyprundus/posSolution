                                ...
                                @if($roles->count() > 0)
                                    @foreach($roles as $role)
                                        <tr>
                                            ...
                                            <td>
                                                @if($role->name != 'admin')
                                                    {!! edit_btn('settings.roles.edit', $role->uuid) !!}
                                                    <a class="btn btn-warning btn-xs" data-toggle="ajax-modal" data-rel="tooltip" data-placement="top" href="{{route('settings.roles.show', $role->uuid)}}" title="{{trans("application.permissions")}}"><i class="fa fa-cog"></i></a>
                                                    {!! delete_btn('settings.roles.destroy', $role->uuid) !!}
                                                @endif
                                            </td>
                                        </tr>
                                    @endforeach
